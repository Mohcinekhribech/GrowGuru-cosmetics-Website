...
class productModel extends db{
    ...
    function getProduct($id){
        $stmt = db::connection()->prepare('select * from product where productId ='.$id.' Limit 1');
        $stmt->execute();
        $result=$stmt->fetchAll();
        return $result;
    }
    function delete($id){
        $stmt = db::connection()->prepare('delete from product where productId = '.$id);
        $stmt->execute();
    }
    ...
    function update($id,$post,$bool){
        ...
        if($bool){
            $stmt = db::connection()->prepare('UPDATE product SET productName = :productName, productPic =  :productPic, productPrice= :productPrice WHERE productId = '.$id);
            ...
            $stmt = db::connection()->prepare('UPDATE product SET productName = :productName, productPrice= :productPrice WHERE productId = '.$id);
            $stmt->bindParam(':productName', $post['productName'], PDO::PARAM_STR);
            $stmt->bindParam(':productPrice', $post['productPrice'], PDO::PARAM_STR);
            $stmt->execute();
            $stmt = null;
